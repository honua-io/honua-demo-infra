#!/usr/bin/env python3
"""Validate the exact successful helper-v3 result and bind its receipt."""
...
SCHEMA = "honua-candidate-preflight-v3-invocation-receipt-v1"
...
RUNTIME = load_module("candidate_preflight_v3_runtime", ROOT / "scripts" / "assert-candidate-preflight-v3-runtime.py")
...
def load(path: Path) -> dict:
    try:
        ...
        raise RuntimeError("v3 invocation artifact is missing or malformed") from exc
    require(isinstance(value, dict), "v3 invocation artifact must be an object")
    return value
...
def assert_invocation(metadata: dict, payload: dict) -> None:
    ...
    require(metadata == {"StatusCode": 200, "ExecutedVersion": "3"}, "v3 invoke metadata drifted or contains FunctionError")
    expected = {
    ...
    require(payload == expected, "v3 payload, pending 092-105 set, digest, or checks drifted")
...
def validate_receipt(receipt: dict) -> None:
    ...
    require(set(receipt) == keys, "v3 invocation receipt keyset drifted")
    require(receipt["schema"] == SCHEMA and receipt["executedVersion"] == "3", "v3 invocation receipt identity drifted")
    require(receipt["deploymentSha"] == RUNTIME.GOVERNANCE.DEPLOYMENT_SHA and receipt["applyEvidenceManifestSha256"] == RUNTIME.GOVERNANCE.APPLY_EVIDENCE_MANIFEST_SHA256, "v3 invocation apply binding drifted")
    require(receipt["pendingScriptsSha256"] == PENDING_DIGEST, "v3 invocation pending digest drifted")
    require(receipt["candidateImageDigest"] == RUNTIME.IMAGE_DIGEST and receipt["candidateSourceCommit"] == "7a29ce0cb4b862b7e58bd58c42e96dcc5e16ccad", "v3 invocation candidate provenance drifted")
    for key in ("runtimeReceiptSha256", "ecrEvidenceSha256", "invocationMetadataSha256", "invocationPayloadSha256"):
        require(RUNTIME.SHA256_PATTERN.fullmatch(receipt[key]) is not None, f"v3 invocation {key} invalid")
...
def build_receipt(metadata_path: Path, payload_path: Path, runtime_path: Path, evidence_path: Path) -> dict:
    ...
    require(runtime["ecrEvidenceSha256"] == sha256(evidence_path), "v3 invocation ECR binding drifted")
...
def main() -> None:
    ...
    if args.mode == "create":
        ...
        require(actual == expected, "v3 invocation artifacts or runtime receipt drifted")
    print(f"candidate-preflight v3 invocation result {args.mode}: PASS")
