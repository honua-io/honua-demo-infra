...
if [[ $# -ne 2 ]]; then echo "usage: $0 GOVERNANCE_SHA DEPLOYMENT_EVIDENCE_DIR" >&2; exit 64; fi
readonly GOVERNANCE_SHA="$1" DEPLOYMENT_EVIDENCE_DIR="$2"
readonly SNAPSHOT_ID="honua-demo-pre-092-105-e0ee6b49e116"
readonly DB_ID="honua-demo-demo-postgres"
readonly SERVING_ARN="arn:aws:lambda:us-west-2:585192672263:function:honua-demo-demo-honua:42"
...
audit() {
  ...
  aws lambda get-function --function-name "$SERVING_ARN" --output json | python scripts/assert-db-migration-runtime.py sanitize serving > "$EVIDENCE_DIR/$prefix-serving.json" || status=1
...
audit pre
python scripts/assert-db-migration-runtime.py create --runner "$EVIDENCE_DIR/pre-runner.json" --serving "$EVIDENCE_DIR/pre-serving.json" --live "$EVIDENCE_DIR/pre-live.json" --db "$EVIDENCE_DIR/pre-db.json" --state "$STATE" --receipt "$EVIDENCE_DIR/runtime.json"
set -o noclobber
...
audit post || status=1
python scripts/assert-db-migration-runtime.py verify --runner "$EVIDENCE_DIR/post-runner.json" --serving "$EVIDENCE_DIR/post-serving.json" --live "$EVIDENCE_DIR/post-live.json" --db "$EVIDENCE_DIR/post-db.json" --state "$STATE" --receipt "$EVIDENCE_DIR/runtime.json" || status=1
