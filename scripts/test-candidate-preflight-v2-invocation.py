...
PATH = ROOT / "scripts" / "assert-candidate-preflight-v2-invocation.py"
spec = importlib.util.spec_from_file_location("v2_invocation_test", PATH)
...
class InvocationTests(unittest.TestCase):
    def test_exact_success_passes(self):
        MODULE.assert_invocation({"StatusCode": 200, "ExecutedVersion": "2"}, payload())
    ...
    def test_transport_failure_versions_contract_and_digest_fail(self):
        cases = {
            ...
            "helper :3": ({"StatusCode": 200, "ExecutedVersion": "3"}, lambda p: None),
            "function error": ({"StatusCode": 200, "ExecutedVersion": "2", "FunctionError": "Unhandled"}, lambda p: None),
            "failed result": ({"StatusCode": 200, "ExecutedVersion": "2"}, lambda p: p.update(status="failed", failure="internal-failure")),
            "contract": ({"StatusCode": 200, "ExecutedVersion": "2"}, lambda p: p["migration"].update(phase="Contract")),
            "pending digest": ({"StatusCode": 200, "ExecutedVersion": "2"}, lambda p: p["migration"].update(pendingScriptsSha256="0" * 64)),
            "checks": ({"StatusCode": 200, "ExecutedVersion": "2"}, lambda p: p.update(checks=list(reversed(p["checks"])))),
        }
