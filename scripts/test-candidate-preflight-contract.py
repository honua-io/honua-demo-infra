...
class CandidatePreflightContractTests(unittest.TestCase):
    ...
    def test_handler_contract_has_fixed_event_and_sanitized_invoke(self):
        ...
        self.assertIn('backup = value.get("backupHook")', handler)
        self.assertNotIn('if "backupHook" not in value:', handler)
        self.assertIn("if backup is None:", handler)
        self.assertIn('backup.get("requiredForPendingSet") is not False', handler)
        self.assertIn('backup.get("pendingContractScripts") != []', handler)
    ...
    def test_runbook_keeps_execution_manual_and_non_mutating(self):
        ...
        self.assertIn("immutable helper version `2`", runbook)
        self.assertIn("historical version `1`", runbook)
        self.assertIn("AWS provider-computed `code_sha256`", runbook)
        self.assertIn("`source_code_size` must remain", runbook)
        self.assertIn("exact historical version-1 values until publication", runbook)
