#!/usr/bin/env python3
"""Audit immutable helper :3 plus the app/alias/ECR pins for one invocation."""
...
QUALIFIED_ARN = f"arn:aws:lambda:{REGION}:{ACCOUNT}:function:{NAME}:3"
REVISION_ID = "8114746a-223f-4358-a260-bd5699d7f992"
CODE_SHA256 = "kp5S3LTvL8L2csu0yvP9zKwUv+0RP0Q++lgnTobCYgU="
CODE_SIZE = 5715
...
HANDLER_SHA256 = "69a59299be3c49530ed04bce9a0bfa53a79d63b0b23dcffcfd9f65c9bde217a1"
CLASSIFICATION_SHA256 = "285b41bcc8b207b234b3ecfdeba7bae88b47920bffcbf0453fa4d099b585b579"
SCHEMA = "honua-candidate-preflight-v3-runtime-receipt-v1"
...
GOVERNANCE = load_module("candidate_preflight_v3_governance", ROOT / "scripts" / "candidate-preflight-v3-governance-receipt.py")
...
def load(path: Path) -> dict:
    try:
        ...
        raise RuntimeError("v3 runtime audit input is missing or malformed") from exc
    require(isinstance(value, dict), "v3 runtime audit input must be an object")
    return value
...
def validate_receipt(receipt: dict, governance_sha: str, governance_receipt_sha: str) -> None:
    ...
    require(set(receipt) == keys, "v3 runtime receipt keyset drifted")
    require(receipt["schema"] == SCHEMA, "v3 runtime receipt schema drifted")
    require(receipt["governanceSha"] == governance_sha and receipt["deploymentSha"] == GOVERNANCE.DEPLOYMENT_SHA, "v3 runtime provenance drifted")
    require(receipt["applyEvidenceManifestSha256"] == GOVERNANCE.APPLY_EVIDENCE_MANIFEST_SHA256, "v3 runtime apply receipt drifted")
    require(receipt["governanceReceiptSha256"] == governance_receipt_sha, "v3 runtime governance receipt drifted")
    require(SHA256_PATTERN.fullmatch(receipt["ecrEvidenceSha256"]) is not None, "v3 runtime ECR hash invalid")
    require(receipt["stateLineage"] == GOVERNANCE.STATE_LINEAGE and receipt["stateSerial"] == GOVERNANCE.STATE_SERIAL, "v3 runtime state binding drifted")
    require(receipt["qualifiedArn"] == QUALIFIED_ARN and receipt["version"] == "3", "v3 runtime helper identity drifted")
    require(receipt["revisionId"] == REVISION_ID and receipt["codeSha256"] == CODE_SHA256 and receipt["codeSize"] == CODE_SIZE, "v3 runtime helper artifact drifted")
    require(receipt["roleArn"] == ROLE_ARN and receipt["policyName"] == POLICY_NAME and SECRET_PATTERN.fullmatch(receipt["secretArn"]), "v3 runtime IAM/secret identity drifted")
    require(receipt["candidateVersion"] == APP_VERSION and receipt["candidateRevisionId"] == APP_REVISION_ID and receipt["candidateImageDigest"] == IMAGE_DIGEST, "v3 runtime candidate pins drifted")
    require(receipt["liveVersion"] == LIVE_VERSION and receipt["liveRevisionId"] == LIVE_REVISION_ID, "v3 runtime live pins drifted")
...
def audit(args) -> dict:
    ...
    require(function.get("FunctionName") == NAME and function.get("FunctionArn") == QUALIFIED_ARN and function.get("Version") == "3", "runtime is not exact helper :3")
    require(function.get("RevisionId") == REVISION_ID and function.get("CodeSha256") == CODE_SHA256 and function.get("CodeSize") == CODE_SIZE, "helper :3 revision/code drifted")
    require(function.get("State") == "Active" and function.get("LastUpdateStatus") == "Successful", "helper :3 is not active/successful")
    require(function.get("Runtime") == "python3.13" and function.get("Handler") == "handler.handler" and function.get("Architectures") == ["arm64"], "helper :3 execution shape drifted")
    require(function.get("Role") == ROLE_ARN and function.get("Timeout") == 120 and function.get("MemorySize") == 128, "helper :3 role/resource bounds drifted")
    require(function.get("Environment", {}).get("Variables") == expected_environment and SECRET_PATTERN.fullmatch(secret_arn), "helper :3 environment drifted")
    require(not function.get("Layers") and not function.get("FileSystemConfigs") and not function.get("DeadLetterConfig", {}).get("TargetArn"), "helper :3 has an attached execution surface")
    vpc = function.get("VpcConfig", {})
    require(not vpc.get("SubnetIds") and not vpc.get("SecurityGroupIds") and not vpc.get("VpcId"), "helper :3 has VPC attachment")
...
def main() -> None:
    ...
    if args.mode == "create":
        ...
        require(actual == expected, "helper/app/alias/ECR differ from v3 runtime receipt")
    print(f"candidate-preflight v3 runtime audit {args.mode}: PASS")
