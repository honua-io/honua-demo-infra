...
PATH = ROOT / "scripts" / "assert-candidate-preflight-v3-runtime.py"
spec = importlib.util.spec_from_file_location("v3_runtime_test", PATH)
...
def receipt():
    return {
        ...
        "governanceReceiptSha256": "1" * 64, "ecrEvidenceSha256": "3" * 64,
        "stateLineage": MODULE.GOVERNANCE.STATE_LINEAGE, "stateSerial": 4,
        "qualifiedArn": MODULE.QUALIFIED_ARN, "version": "3", "revisionId": MODULE.REVISION_ID,
...
class RuntimeTests(unittest.TestCase):
    def test_exact_v3_receipt_passes(self):
    ...
    def test_wrong_version_revision_hash_apply_and_app_pins_fail(self):
        mutations = {
            ...
            "helper :2": lambda r: r.update(version="2", qualifiedArn=MODULE.QUALIFIED_ARN[:-1] + "2"),
            "helper :4": lambda r: r.update(version="4", qualifiedArn=MODULE.QUALIFIED_ARN[:-1] + "4"),
            ...
            "state serial": lambda r: r.update(stateSerial=5),
