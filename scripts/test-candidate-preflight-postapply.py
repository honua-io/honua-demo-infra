...
class CandidatePreflightPostapplyTests(unittest.TestCase):
    ...
    def test_every_action_or_graph_mutation_fails_closed(self) -> None:
        mutations = {
            ...
            "resource replace path": lambda p: self.change(p, "aws_lambda_function.candidate_preflight")["change"].update(replace_paths=[["runtime"]]),
    ...
    def test_every_provider_readback_mutation_fails_closed(self) -> None:
        ...
        mutations = {
            ...
            "drift replace path": lambda p: self.drift(p, policy_address)["change"].update(replace_paths=[["inline_policy"]]),
            ...
            "policy duplicate key": lambda p: self.drift(p, policy_address)["change"]["after"]["inline_policy"][0].update(policy="{\"Version\":\"2012-10-17\",\"Statement\":[],\"Statement\":[{\"Sid\":\"ReadExactLiveAlias\"}]}"),
            ...
            "policy sensitivity count": lambda p: self.drift(p, policy_address)["change"]["after_sensitive"].update(inline_policy=[{}, {}]),
            "policy sensitivity other field": lambda p: self.drift(p, policy_address)["change"]["after_sensitive"].update(tags={}),
            ...
            "layers before sensitivity": lambda p: self.drift(p, lambda_address)["change"]["before_sensitive"].update(layers=[]),
            "layers after sensitivity": lambda p: self.drift(p, lambda_address)["change"]["after_sensitive"].update(layers=[True]),
            "layers sensitivity other field": lambda p: self.drift(p, lambda_address)["change"]["after_sensitive"].update(tags={}),
        }
        for label, mutation in mutations.items():
            self.assert_rejected(label, mutation)
