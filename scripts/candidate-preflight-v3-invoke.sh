...
readonly EVIDENCE_DIR="$HOME/.honua-runtime-proof/candidate-preflight-v3-invocation-$GOVERNANCE_SHA"
readonly APPLY_MANIFEST="$APPLY_EVIDENCE_DIR/final-artifacts.sha256"
readonly QUALIFIED_ARN="arn:aws:lambda:us-west-2:585192672263:function:honua-demo-demo-candidate-preflight:3"
...
runtime_audit() {
  ...
  python scripts/assert-candidate-preflight-v3-runtime.py "$mode" \
    ...
    --governance-receipt "$EVIDENCE_DIR/governance-receipt-v3.json" \
    --ecr-evidence "$EVIDENCE_DIR/$prefix-ecr-evidence.json" \
    --governance-sha "$GOVERNANCE_SHA" \
    --receipt "$EVIDENCE_DIR/runtime-receipt-v3.json"
}

governance_verify() {
  python scripts/candidate-preflight-v3-governance-receipt.py verify --governance-sha "$GOVERNANCE_SHA" --apply-evidence-manifest "$APPLY_MANIFEST" --evidence-dir "$EVIDENCE_DIR" --receipt "$EVIDENCE_DIR/governance-receipt-v3.json"
}

# Every gate before the marker is fail-fast and cannot invoke Lambda.
python scripts/candidate-preflight-v3-governance-receipt.py create --governance-sha "$GOVERNANCE_SHA" --apply-evidence-manifest "$APPLY_MANIFEST" --evidence-dir "$EVIDENCE_DIR" --receipt "$EVIDENCE_DIR/governance-receipt-v3.json"
governance_verify
capture_helper_audit preinvoke
capture_app_audit preinvoke
capture_ecr_audit preinvoke
runtime_audit create preinvoke
sha256sum "$EVIDENCE_DIR/runtime-receipt-v3.json" > "$EVIDENCE_DIR/runtime-receipt-v3.sha256"
sha256sum --check "$EVIDENCE_DIR/runtime-receipt-v3.sha256"
governance_verify
...
set -o noclobber
printf '{"schema":"honua-candidate-preflight-v3-attempt-v1","attempt":1,"governanceSha":"%s","qualifiedArn":"%s"}\n' "$GOVERNANCE_SHA" "$QUALIFIED_ARN" > "$EVIDENCE_DIR/invocation-attempt-v3.json"
set +o noclobber
...
if ! aws lambda invoke \
  ...
  "$EVIDENCE_DIR/invocation-payload-v3.json" \
  > "$EVIDENCE_DIR/invocation-metadata-v3.json"; then
  invocation_status=1
fi
if ! python scripts/assert-candidate-preflight-v3-invocation.py create --metadata "$EVIDENCE_DIR/invocation-metadata-v3.json" --payload "$EVIDENCE_DIR/invocation-payload-v3.json" --runtime-receipt "$EVIDENCE_DIR/runtime-receipt-v3.json" --ecr-evidence "$EVIDENCE_DIR/preinvoke-ecr-evidence.json" --receipt "$EVIDENCE_DIR/invocation-receipt-v3.json"; then invocation_status=1; fi
...
if ! python scripts/assert-candidate-preflight-v3-invocation.py verify --metadata "$EVIDENCE_DIR/invocation-metadata-v3.json" --payload "$EVIDENCE_DIR/invocation-payload-v3.json" --runtime-receipt "$EVIDENCE_DIR/runtime-receipt-v3.json" --ecr-evidence "$EVIDENCE_DIR/postinvoke-ecr-evidence.json" --receipt "$EVIDENCE_DIR/invocation-receipt-v3.json"; then invocation_status=1; fi
if ! sha256sum "$EVIDENCE_DIR"/*.json > "$EVIDENCE_DIR/invocation-artifacts-v3.sha256"; then invocation_status=1; fi
